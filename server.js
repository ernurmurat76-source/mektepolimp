...
const ROOT=__dirname,DB_FILE=path.join(ROOT,'data','db.json'),PUBLIC=path.join(ROOT,'public');
...
function db(){return JSON.parse(fs.readFileSync(DB_FILE,'utf8'))}function save(x){fs.writeFileSync(DB_FILE,JSON.stringify(x,null,2))}
...
async function api(req,res,url){
  ...
  if(req.method==='POST'&&url.pathname==='/api/admin/ai-generate'){
    if(!process.env.OPENAI_API_KEY)return out(res,400,{error:'OPENAI_API_KEY орнатылмаған. README нұсқаулығын қараңыз'});
    ...
    const rr=await fetch('https://api.openai.com/v1/responses',{method:'POST',headers:{authorization:'Bearer '+process.env.OPENAI_API_KEY,'content-type':'application/json'},body:JSON.stringify({model:process.env.OPENAI_MODEL||'gpt-6-astra',input:[{role:'system',content:'Сіз тәжірибелі пән мұғалімі және олимпиада тапсырмаларын құрастырушысыз.'},{role:'user',content:prompt}],text:{format:{type:'json_schema',name:'olympiad_questions',strict:true,schema}}})});
    const data=await rr.json();if(!rr.ok)return out(res,502,{error:data.error?.message||'AI қызметі жауап бермеді'});const txt=data.output_text||data.output?.flatMap(x=>x.content||[]).find(x=>x.type==='output_text')?.text;if(!txt)return out(res,502,{error:'AI жауабы бос'});const generated=JSON.parse(txt).questions.map(q=>({...q,id:crypto.randomUUID(),grade,subject,points:1,draft:true}));d.questions.push(...generated);save(d);return out(res,200,{count:generated.length,questions:generated});
  }
