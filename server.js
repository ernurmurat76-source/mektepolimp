...
function randomQuestionIds(items,limit){
  const a=items.map(q=>q.id);
...
function importedQuestion(row){
  const grade=Number(row[0]),subject=String(row[1]||'').trim(),text=String(row[2]||'').trim(),options=row.slice(3,7).map(x=>String(x||'').trim());
  const raw=String(row[7]??'').trim().toUpperCase(),correct=['A','B','C','D'].includes(raw)?'ABCD'.indexOf(raw):Number(raw)-1;
  if(![5,6,7,8,9,10,11].includes(grade)||!subjects.includes(subject)||!text||options.some(x=>!x)||correct<0||correct>3)return null;
  return {id:crypto.randomUUID(),grade,subject,text,options,correct,points:1};
}
async function api(req,res,url){
  ...
  if(req.method==='POST'&&url.pathname==='/api/student/login'){
    ...
    return out(res,200,{token:issueSession(s.id),student:cleanStudent(s)});
  }
  if(req.method==='GET'&&url.pathname==='/api/exam'){
    const sid=sessionValue(req),s=d.students.find(x=>x.id===sid);if(!s)return out(res,401,{error:'Қайта кіріңіз'});
    const old=d.attempts.find(x=>x.studentId===s.id&&x.submittedAt);if(old)return out(res,409,{error:'Сіз тестті тапсырып қойдыңыз'});
    const pool=d.questions.filter(q=>q.grade===s.grade&&q.subject===s.subject),limit=25;
    let a=d.attempts.find(x=>x.studentId===s.id&&!x.submittedAt);
    if(!a){a={id:crypto.randomUUID(),studentId:s.id,startedAt:new Date().toISOString(),answers:{},questionIds:randomQuestionIds(pool,limit)};d.attempts.push(a);save(d)}
    else if(!Array.isArray(a.questionIds)){a.questionIds=randomQuestionIds(pool,limit);save(d)}
    const chosen=new Set(a.questionIds),qs=pool.filter(q=>chosen.has(q.id)).sort((x,y)=>a.questionIds.indexOf(x.id)-a.questionIds.indexOf(y.id)).map(({correct,...q})=>q);
    return out(res,200,{attemptId:a.id,student:cleanStudent(s),durationMinutes:d.settings.durationMinutes,startedAt:a.startedAt,answers:a.answers||{},questions:qs});
  }
  if(req.method==='POST'&&url.pathname==='/api/exam/save'){
    const sid=sessionValue(req),s=d.students.find(x=>x.id===sid);if(!s)return out(res,401,{error:'Қайта кіріңіз'});const b=await body(req),a=d.attempts.find(x=>x.id===b.attemptId&&x.studentId===sid&&!x.submittedAt);if(!a)return out(res,409,{error:'Бұл тест аяқталған'});const chosen=new Set(a.questionIds||[]);for(const [id,value] of Object.entries(b.answers||{}))if(chosen.has(id)&&Number.isInteger(Number(value))&&Number(value)>=0&&Number(value)<=3)a.answers[id]=Number(value);save(d);return out(res,200,{ok:true});
  }
  if(req.method==='POST'&&url.pathname==='/api/exam/submit'){
    const sid=sessionValue(req),s=d.students.find(x=>x.id===sid);if(!s)return out(res,401,{error:'Қайта кіріңіз'});const b=await body(req),a=d.attempts.find(x=>x.id===b.attemptId&&x.studentId===sid&&!x.submittedAt);if(!a)return out(res,409,{error:'Бұл тест жіберілген'});
    const chosen=new Set(Array.isArray(a.questionIds)?a.questionIds:[]),qs=d.questions.filter(q=>q.grade===s.grade&&q.subject===s.subject&&(!chosen.size||chosen.has(q.id))).sort((x,y)=>(a.questionIds||[]).indexOf(x.id)-(a.questionIds||[]).indexOf(y.id));let score=0,total=0;for(const q of qs){total+=q.points||1;if(Number(b.answers?.[q.id])===q.correct)score+=q.points||1}Object.assign(a,{answers:b.answers||{},score,total,submittedAt:new Date().toISOString()});save(d);const review=qs.map(q=>{const selected=Number.isInteger(Number(b.answers?.[q.id]))?Number(b.answers[q.id]):null;return {id:q.id,text:q.text,options:q.options,selected,correct:q.correct,isCorrect:selected===q.correct}});return out(res,200,{score,total,percent:total?Math.round(score/total*100):0,review});
  }
  ...
  if(req.method==='POST'&&url.pathname==='/api/admin/questions/import'){
    ...
    if(!buf.length||buf.length>10e6)return out(res,400,{error:'Файл бос немесе 10 МБ-тан үлкен'});
    let rows=[];
    if(name.endsWith('.xlsx')||name.endsWith('.xls')){
      const XLSX=require('xlsx'),book=XLSX.read(buf,{type:'buffer'}),sheet=book.Sheets[book.SheetNames[0]];
      rows=XLSX.utils.sheet_to_json(sheet,{header:1,defval:''}).slice(1);
    }else if(name.endsWith('.docx')){
      const mammoth=require('mammoth'),html=(await mammoth.convertToHtml({buffer:buf})).value;
      rows=[...html.matchAll(/<tr[^>]*>([\s\S]*?)<\/tr>/gi)].map(m=>[...m[1].matchAll(/<t[dh][^>]*>([\s\S]*?)<\/t[dh]>/gi)].map(c=>decodeHtml(c[1]))).slice(1);
    }else return out(res,400,{error:'Тек .xlsx, .xls немесе .docx файлын таңдаңыз'});
    const questions=rows.slice(0,5000).map(importedQuestion).filter(Boolean);
    if(!questions.length)return out(res,400,{error:'Жарамды сұрақ табылмады. Бағандар: Сынып, Пән, Сұрақ, A, B, C, D, Дұрыс жауап'});
    d.questions.push(...questions);save(d);return out(res,200,{count:questions.length,skipped:rows.length-questions.length});
  }
